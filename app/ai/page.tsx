...
const SYSTEM = `You are an accounting reasoning assistant. Analyze the problem and return ONLY JSON: {"analysis":"","assumptions":[],"journal":[{"account":"","debit":0,"credit":0,"explanation":""}],"checks":[],"outputs":[]}. Use standard double-entry rules. Do not invent facts. State assumptions. Journal must balance. Numbers must be plain numbers.`;
const CHAT_SYSTEM = `You are a concise accounting tutor. Explain debit/credit logic and accounting concepts accurately. Use short answers unless detail is requested. Do not fabricate facts.`;
...
export default function AIWorkbench() {
  ...
  async function initialize(auto = false) {
    ...
    try {
      ...
      if (previous) {
        ...
        if (previous.status === "downloading") setStatusText(`Resuming cached model from ${previous.progress}%…`);
      ...
      setEngine(next);
      setStatus("ready"); setProgress(100); setStatusText("Local LLM ready — inference runs off the UI thread");
      saveWebLLMState({ modelId: MODEL, status: "ready", progress: 100, text: "Local LLM ready", updatedAt: Date.now() });
    } catch (e: any) {
      setGpuInfo(e?.message?.includes("GPU") ? "No compatible GPU adapter" : gpuInfo);
      setStatus("error"); setStatusText("Local LLM could not start");
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setStatusText(saved.status === "ready" ? "Cached model found — restoring…" : saved.status === "downloading" ? `Previous download found — resuming from ${saved.progress}%…` : saved.text);
    }
  ...
  async function complete(messages: any[], temperature = 0.1, maxTokens = MAX_GENERATION_TOKENS) {
    if (!engine) throw new Error("Initialize the local model first.");
    const result = await engine.chat.completions.create({ messages, temperature, max_tokens: maxTokens });
    return result?.choices?.[0]?.message?.content || "";
  }

  async function solve() {
    if (!problem.trim()) return setError("Enter an accounting problem first.");
    setBusy(true); setError(""); setSolution(null); setBusyStage("Reading the accounting problem…");
    try {
      setBusyStage("Identifying transactions and accounts…");
      const raw = await complete([{ role: "system", content: SYSTEM }, { role: "user", content: `Solve this accounting problem:\n\n${problem.trim()}` }], 0.05, 700);
      setBusyStage("Building the journal entry…");
      ...
      setBusyStage("Validating that debits equal credits…");
  ...
  async function sendChat() {
    ...
    try {
      const messages = [{ role: "system", content: CHAT_SYSTEM }, ...next.map((m) => ({ role: m.role, content: m.text }))];
      const raw = await complete(messages, 0.2, 256);
    ...
    <main className="ai-shell">
      ...
      <section className="ai-grid">
        <aside className="ai-card model-card glass-card">
          <div className="eyebrow">LOCAL RUNTIME</div><h2>Qwen 0.5B</h2><p className="muted">Inference runs in a dedicated worker so generation does not block the page's main UI thread. WebGPU is still required.</p>
        ...
        <section className="ai-card solver-card glass-card">
          ...
          {busy && <div className="ai-activity"><div className="activity-orb"/><div><strong>{busyStage || "Working…"}</strong><span>Model is running in a background worker. The interface should remain responsive.</span></div><div className="typing"><i/><i/><i/></div></div>}
